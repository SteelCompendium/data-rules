---
action_type: feature
class: shadow
feature_type: feature
file_basename: cinder-step
file_dpath: feature/shadow/level-8
item_id: cinder-step
item_name: Cinder Step
level: "8"
name: Cinder Step
scc: mcdm.heroes.v1/feature.shadow.level-8/cinder-step
source: mcdm.heroes.v1
type: feature
---

```ds-feature
effects:
    - effect: Whenever you willingly move, you can [teleport](scc:mcdm.heroes.v1/movement/teleport). When you [teleport](scc:mcdm.heroes.v1/movement/teleport) this way, it counts as using a shadow ability for the purpose of using your [Burning Ash](scc:mcdm.heroes.v1/feature.shadow.level-2/burning-ash) and [Trail of Cinders](scc:mcdm.heroes.v1/feature.shadow.level-5/trail-of-cinders) features.
feature_type: feature
metadata:
    class: shadow
    level: "8"
    name: Cinder Step
    scc: mcdm.heroes.v1/feature.shadow.level-8/cinder-step
    type: feature
name: Cinder Step
type: feature
```
