---
action_type: Maneuver
class: conduit
cost: 9 Piety
cost_amount: "9"
cost_resource: Piety
distance: 4 [burst](scc:mcdm.heroes.v1/rule.combat/burst)
effect: Each target is invisible until the start of your next [turn](scc:mcdm.heroes.v1/rule.combat/turn).
feature_type: ability
file_basename: invocation-of-mystery
file_dpath: feature/ability/conduit/level-6
flavor: '"Now you see us..."'
item_id: invocation-of-mystery
item_name: Invocation of Mystery
keywords:
    - Area
    - Magic
level: "6"
name: Invocation of Mystery
scc: mcdm.heroes.v1/feature.ability.conduit.level-6/invocation-of-mystery
source: mcdm.heroes.v1
target: Self and each ally in the area
type: ability
---

```ds-feature
cost: 9 Piety
distance: 4 [burst](scc:mcdm.heroes.v1/rule.combat/burst)
effects:
    - effect: Each target is invisible until the start of your next [turn](scc:mcdm.heroes.v1/rule.combat/turn).
feature_type: ability
flavor: '"Now you see us..."'
keywords:
    - Area
    - Magic
metadata:
    action_type: Maneuver
    class: conduit
    cost: 9 Piety
    distance: 4 [burst](scc:mcdm.heroes.v1/rule.combat/burst)
    effect: Each target is invisible until the start of your next [turn](scc:mcdm.heroes.v1/rule.combat/turn).
    flavor: '"Now you see us..."'
    keywords:
        - Area
        - Magic
    level: "6"
    name: Invocation of Mystery
    scc: mcdm.heroes.v1/feature.ability.conduit.level-6/invocation-of-mystery
    target: Self and each ally in the area
    type: ability
name: Invocation of Mystery
target: Self and each ally in the area
type: feature
usage: Maneuver
```
