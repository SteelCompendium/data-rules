---
action_type: Maneuver
class: conduit
cost: 11 Piety
cost_amount: "11"
cost_resource: Piety
distance: Ranged 10
effect: The target has an additional main action they can use on their next turn, and gains 3 of their [Heroic Resource](scc:mcdm.heroes.v1/rule.resource/heroic-resource).
feature_type: ability
file_basename: alacrity-of-the-heart
file_dpath: feature/ability/conduit/level-9
flavor: You speak inspiring words to a friend and spur them to incredible feats.
item_id: alacrity-of-the-heart
item_name: Alacrity of the Heart
keywords:
    - Magic
    - Ranged
level: "9"
name: Alacrity of the Heart
scc: mcdm.heroes.v1/feature.ability.conduit.level-9/alacrity-of-the-heart
source: mcdm.heroes.v1
target: One ally
type: ability
---

```ds-feature
cost: 11 Piety
distance: Ranged 10
effects:
    - effect: The target has an additional main action they can use on their next turn, and gains 3 of their [Heroic Resource](scc:mcdm.heroes.v1/rule.resource/heroic-resource).
feature_type: ability
flavor: You speak inspiring words to a friend and spur them to incredible feats.
keywords:
    - Magic
    - Ranged
metadata:
    action_type: Maneuver
    class: conduit
    cost: 11 Piety
    distance: Ranged 10
    effect: The target has an additional main action they can use on their next turn, and gains 3 of their [Heroic Resource](scc:mcdm.heroes.v1/rule.resource/heroic-resource).
    flavor: You speak inspiring words to a friend and spur them to incredible feats.
    keywords:
        - Magic
        - Ranged
    level: "9"
    name: Alacrity of the Heart
    scc: mcdm.heroes.v1/feature.ability.conduit.level-9/alacrity-of-the-heart
    target: One ally
    type: ability
name: Alacrity of the Heart
target: One ally
type: feature
usage: Maneuver
```
