---
action_type: Main action
class: conduit
cost: 11 Piety
cost_amount: "11"
cost_resource: Piety
distance: '[Ranged](scc:mcdm.heroes.v1/rule.combat/ranged) 10'
effect: You can target yourself instead of one ally with this ability. Each target can spend any number of [Recoveries](scc:mcdm.heroes.v1/rule.health/recoveries), can end any effects on them that are ended by a [saving throw](scc:mcdm.heroes.v1/rule.general/saving-throw) or that end at the end of their [turn](scc:mcdm.heroes.v1/rule.combat/turn), and can stand up if they [are prone](scc:mcdm.heroes.v1/condition/prone).
feature_type: ability
file_basename: radiance-of-grace
file_dpath: feature/ability/conduit/level-9
flavor: Intense light is emitted from your body, healing your allies.
item_id: radiance-of-grace
item_name: Radiance of Grace
keywords:
    - Magic
    - '[Ranged](scc:mcdm.heroes.v1/rule.combat/ranged)'
level: "9"
name: Radiance of Grace
scc: mcdm.heroes.v1/feature.ability.conduit.level-9/radiance-of-grace
source: mcdm.heroes.v1
target: Four allies
type: ability
---

```ds-feature
cost: 11 Piety
distance: '[Ranged](scc:mcdm.heroes.v1/rule.combat/ranged) 10'
effects:
    - effect: You can target yourself instead of one ally with this ability. Each target can spend any number of [Recoveries](scc:mcdm.heroes.v1/rule.health/recoveries), can end any effects on them that are ended by a [saving throw](scc:mcdm.heroes.v1/rule.general/saving-throw) or that end at the end of their [turn](scc:mcdm.heroes.v1/rule.combat/turn), and can stand up if they [are prone](scc:mcdm.heroes.v1/condition/prone).
feature_type: ability
flavor: Intense light is emitted from your body, healing your allies.
keywords:
    - Magic
    - '[Ranged](scc:mcdm.heroes.v1/rule.combat/ranged)'
metadata:
    action_type: Main action
    class: conduit
    cost: 11 Piety
    distance: '[Ranged](scc:mcdm.heroes.v1/rule.combat/ranged) 10'
    effect: You can target yourself instead of one ally with this ability. Each target can spend any number of [Recoveries](scc:mcdm.heroes.v1/rule.health/recoveries), can end any effects on them that are ended by a [saving throw](scc:mcdm.heroes.v1/rule.general/saving-throw) or that end at the end of their [turn](scc:mcdm.heroes.v1/rule.combat/turn), and can stand up if they [are prone](scc:mcdm.heroes.v1/condition/prone).
    flavor: Intense light is emitted from your body, healing your allies.
    keywords:
        - Magic
        - '[Ranged](scc:mcdm.heroes.v1/rule.combat/ranged)'
    level: "9"
    name: Radiance of Grace
    scc: mcdm.heroes.v1/feature.ability.conduit.level-9/radiance-of-grace
    target: Four allies
    type: ability
name: Radiance of Grace
target: Four allies
type: feature
usage: Main action
```
