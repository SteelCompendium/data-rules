---
action_type: Maneuver
class: talent
distance: Ranged 10
effect: The next [ability roll](scc:mcdm.heroes.v1/rule.dice/ability-roll) an ally makes against the target before the start of your next turn gains an [edge](scc:mcdm.heroes.v1/rule.dice/edge).
feature_type: ability
file_basename: remote-assistance
file_dpath: feature/ability/talent/level-1
flavor: An ally gains the benefit of your intellect.
item_id: remote-assistance
item_name: Remote Assistance
keywords:
    - Psionic
    - Ranged
level: "1"
name: Remote Assistance
scc: mcdm.heroes.v1/feature.ability.talent.level-1/remote-assistance
source: mcdm.heroes.v1
spend: '1 Clarity: You target one additional creature or object.'
target: One creature or object
type: ability
---

```ds-feature
distance: Ranged 10
effects:
    - effect: The next [ability roll](scc:mcdm.heroes.v1/rule.dice/ability-roll) an ally makes against the target before the start of your next turn gains an [edge](scc:mcdm.heroes.v1/rule.dice/edge).
    - effect: '1 Clarity: You target one additional creature or object.'
      name: Spend
feature_type: ability
flavor: An ally gains the benefit of your intellect.
keywords:
    - Psionic
    - Ranged
metadata:
    action_type: Maneuver
    class: talent
    distance: Ranged 10
    effect: The next [ability roll](scc:mcdm.heroes.v1/rule.dice/ability-roll) an ally makes against the target before the start of your next turn gains an [edge](scc:mcdm.heroes.v1/rule.dice/edge).
    flavor: An ally gains the benefit of your intellect.
    keywords:
        - Psionic
        - Ranged
    level: "1"
    name: Remote Assistance
    scc: mcdm.heroes.v1/feature.ability.talent.level-1/remote-assistance
    spend: '1 Clarity: You target one additional creature or object.'
    target: One creature or object
    type: ability
name: Remote Assistance
target: One creature or object
type: feature
usage: Maneuver
```
