---
action_type: feature
class: troubadour
feature_type: trait
file_basename: virtuoso-features
file_dpath: feature/trait/troubadour/level-5
item_id: virtuoso-features
item_name: Virtuoso Features
level: "5"
name: Virtuoso Features
scc: mcdm.heroes.v1/feature.trait.troubadour.level-5/virtuoso-features
source: mcdm.heroes.v1
type: trait
---

```ds-feature
effects:
    - effect: |-
        Choose one of the following features.

        ###### Bolstering Banter

        Once on each of your turns, you can use a free maneuver to exchange words with a target of your current performance, other than yourself. The target can spend a Recovery to gain temporary Stamina equal to their recovery value.

        ###### Medley

        You can maintain two performances at a time using your [Routines](scc:mcdm.heroes.v1/feature.trait.troubadour.level-1/routines) feature.
feature_type: trait
metadata:
    class: troubadour
    level: "5"
    name: Virtuoso Features
    scc: mcdm.heroes.v1/feature.trait.troubadour.level-5/virtuoso-features
    type: trait
name: Virtuoso Features
type: feature
```
