---
action_type: feature
class: conduit
feature_type: feature
file_basename: prayer-of-speed
file_dpath: feature/conduit/level-1
item_id: prayer-of-speed
item_name: Prayer of Speed
level: "1"
name: Prayer of Speed
scc: mcdm.heroes.v1/feature.conduit.level-1/prayer-of-speed
source: mcdm.heroes.v1
type: feature
---

```ds-feature
effects:
    - effect: Your god blesses your flesh and infuses it with divine quickness. You gain a +1 bonus to speed and to the [distance](scc:mcdm.heroes.v1/rule.combat/distance) you can [shift](scc:mcdm.heroes.v1/movement/shifting) when you take the [Disengage](scc:mcdm.heroes.v1/feature.common.move-actions/disengage) move action.
feature_type: feature
metadata:
    class: conduit
    level: "1"
    name: Prayer of Speed
    scc: mcdm.heroes.v1/feature.conduit.level-1/prayer-of-speed
    type: feature
name: Prayer of Speed
type: feature
```
