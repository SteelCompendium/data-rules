---
action_type: feature
class: elementalist
feature_type: feature
file_basename: specialization-triggered-action
file_dpath: feature/elementalist/level-1
item_id: specialization-triggered-action
item_name: Specialization Triggered Action
level: "1"
name: Specialization Triggered Action
scc: mcdm.heroes.v1/feature.elementalist.level-1/specialization-triggered-action
source: mcdm.heroes.v1
type: feature
---

```ds-feature
effects:
    - effect: |-
        Your [elemental specialization](scc:mcdm.heroes.v1/feature.elementalist.level-1/elemental-specialization) grants you a [triggered action](scc:mcdm.heroes.v1/rule.combat/triggered-action), as shown on the Specialization [Triggered Actions](scc:mcdm.heroes.v1/rule.combat/triggered-action) table.

        ###### Elemental Specialization Triggered Actions Table

        | Specialization  | [Triggered Action](scc:mcdm.heroes.v1/rule.combat/triggered-action)          |
        |-----------------|---------------------------|
        | Earth           | [Skin Like Castle Walls](scc:mcdm.heroes.v1/feature.ability.elementalist.level-1/skin-like-castle-walls)    |
        | Fire            | [Explosive Assistance](scc:mcdm.heroes.v1/feature.ability.elementalist.level-1/explosive-assistance)      |
        | Green           | [Breath of Dawn Remembered](scc:mcdm.heroes.v1/feature.ability.elementalist.level-1/breath-of-dawn-remembered) |
        | Void            | [Subtle Relocation](scc:mcdm.heroes.v1/feature.ability.elementalist.level-1/subtle-relocation)         |
feature_type: feature
metadata:
    class: elementalist
    level: "1"
    name: Specialization Triggered Action
    scc: mcdm.heroes.v1/feature.elementalist.level-1/specialization-triggered-action
    type: feature
name: Specialization Triggered Action
type: feature
```
