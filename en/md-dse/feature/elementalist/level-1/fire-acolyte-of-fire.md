---
action_type: feature
class: elementalist
feature_type: feature
file_basename: fire-acolyte-of-fire
file_dpath: feature/elementalist/level-1
item_id: fire-acolyte-of-fire
item_name: 'Fire: Acolyte of Fire'
level: "1"
name: 'Fire: Acolyte of Fire'
scc: mcdm.heroes.v1/feature.elementalist.level-1/fire-acolyte-of-fire
source: mcdm.heroes.v1
type: feature
---

```ds-feature
effects:
    - effect: You become an expert at wielding destructive flames. Your abilities that have the Fire and Magic keywords gain a +1 [bonus](scc:mcdm.heroes.v1/rule.dice/bonuses-and-penalties) to [rolled damage](scc:mcdm.heroes.v1/rule.damage/rolled-damage). Your [Hurl Element](scc:mcdm.heroes.v1/feature.ability.elementalist.level-1/hurl-element) ability (see below) also gains this [bonus](scc:mcdm.heroes.v1/rule.dice/bonuses-and-penalties) when you use it to deal fire damage.
feature_type: feature
metadata:
    class: elementalist
    level: "1"
    name: 'Fire: Acolyte of Fire'
    scc: mcdm.heroes.v1/feature.elementalist.level-1/fire-acolyte-of-fire
    type: feature
name: 'Fire: Acolyte of Fire'
type: feature
```
