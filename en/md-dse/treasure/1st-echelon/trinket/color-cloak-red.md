---
echelon: "1"
effect: While worn, a red Color Cloak grants you fire immunity equal to your level.
file_basename: color-cloak-red
file_dpath: treasure/1st-echelon/trinket
item_id: color-cloak-red
item_name: Color Cloak (Red)
keywords:
    - Magic
    - Neck
name: Color Cloak (Red)
scc: mcdm.heroes.v1/treasure.1st-echelon.trinket/color-cloak-red
source: mcdm.heroes.v1
treasure_type: trinket
type: treasure
---

*This red woolen hooded cloak is emblazoned with a golden Anjali sigil meaning "fire."*

**Keywords:** Magic, Neck

**[Item Prerequisite](scc:mcdm.heroes.v1/rule.downtime/item-prerequisite):** A pint of red ichor, soul chalk

**[Project Source](scc:mcdm.heroes.v1/rule.downtime/project-source):** Licensing agreements in Anjali

**[Project Roll](scc:mcdm.heroes.v1/rule.downtime/project-roll) [Characteristic](scc:mcdm.heroes.v1/rule.character/characteristic):** [Reason](scc:mcdm.heroes.v1/rule.character/reason) or [Intuition](scc:mcdm.heroes.v1/rule.character/intuition)

**Project Goal:** 150

**Effect:** While worn, a red Color Cloak grants you fire immunity equal to your level.

Additionally, when you are targeted by any effect that deals fire damage, you can use a [triggered action](scc:mcdm.heroes.v1/rule.combat/triggered-action) to reduce the damage to 0. If you do so, the fire immunity granted by this cloak becomes fire weakness with the same value until the end of the next round. You can't use this [triggered action](scc:mcdm.heroes.v1/rule.combat/triggered-action) again until this weakness ends.
