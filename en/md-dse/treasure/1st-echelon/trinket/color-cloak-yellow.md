---
echelon: "1"
effect: While worn, a yellow Color Cloak grants you lightning immunity equal to your level.
file_basename: color-cloak-yellow
file_dpath: treasure/1st-echelon/trinket
item_id: color-cloak-yellow
item_name: Color Cloak (Yellow)
keywords:
    - Magic
    - Neck
name: Color Cloak (Yellow)
scc: mcdm.heroes.v1/treasure.1st-echelon.trinket/color-cloak-yellow
source: mcdm.heroes.v1
treasure_type: trinket
type: treasure
---

*This yellow rubbery hooded cloak is emblazoned with a golden Anjali sigil meaning "lightning."*

**Keywords:** Magic, Neck

**[Item Prerequisite](scc:mcdm.heroes.v1/rule.downtime/item-prerequisite):** A pint of yellow ichor, soul chalk

**[Project Source](scc:mcdm.heroes.v1/rule.downtime/project-source):** Licensing agreements in Anjali

**[Project Roll](scc:mcdm.heroes.v1/rule.downtime/project-roll) [Characteristic](scc:mcdm.heroes.v1/rule.character/characteristic):** [Reason](scc:mcdm.heroes.v1/rule.character/reason) or [Intuition](scc:mcdm.heroes.v1/rule.character/intuition)

**Project Goal:** 150

**Effect:** While worn, a yellow Color Cloak grants you lightning immunity equal to your level.

Additionally, when you are targeted by any effect that deals lightning damage, you can use a [triggered action](scc:mcdm.heroes.v1/rule.combat/triggered-action) to cause the next damage-dealing ability you use to deal extra lightning damage equal to your level. Once you deal this extra damage, your lightning immunity becomes lightning weakness with the same value until the end of the next round. You can't use this [triggered action](scc:mcdm.heroes.v1/rule.combat/triggered-action) again until this weakness ends.
